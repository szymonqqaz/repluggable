...
export declare type ScopedStore<S> = Pick<ThrottledStore<S>, 'dispatch' | 'getState' | 'subscribe' | 'flush'>;
export declare type ReactComponentContributor<TProps = {}> = (props?: TProps) => React.ReactNode;
export declare type ReducersMapObjectContributor<TState = {}, TAction extends Redux.AnyAction = Redux.AnyAction> = () => Redux.ReducersMapObject<TState, TAction>;
export declare type ContributionPredicate = () => boolean;
export interface EntryPointTags {
    [name: string]: string;
}
export declare type LazyEntryPointFactory = () => Promise<EntryPoint>;
export declare type ShellsChangedCallback = (shellNames: string[]) => void;
export declare type ShellBoundaryAspect = React.FunctionComponent;
...
export declare type AnyEntryPoint = EntryPoint | LazyEntryPointDescriptor;
export declare type EntryPointOrPackage = AnyEntryPoint | AnyEntryPoint[];
export interface EntryPointOrPackagesMap {
    [name: string]: EntryPointOrPackage;
}
export declare type ExtensionItemFilter<T> = (extensionItem: ExtensionItem<T>) => boolean;
/**
...
export declare type enrichedMemoizationFunction = MemoizeMissHit & AnyFunction & _.MemoizedFunction;
...
export declare type StateObserverUnsubscribe = () => void;
export declare type StateObserver<TSelectorAPI> = (next: TSelectorAPI) => void;
...
export declare type FunctionWithSameArgs<F extends AnyFunction> = (...args: Parameters<F>) => any;
/**
...
export interface Shell extends Pick<AppHost, Exclude<keyof AppHost, 'getStore' | 'log' | 'options'>> {
     ...
     */
    contributeState<TState, TAction extends Redux.AnyAction = Redux.AnyAction>(contributor: ReducersMapObjectContributor<TState, TAction>): void;
    /**
     * Contribute a Redux reducer that will be added to the host store
     ...
     */
    contributeObservableState<TState, TSelector, TAction extends Redux.AnyAction = Redux.AnyAction>(contributor: ReducersMapObjectContributor<TState, TAction>, selectorFactory: (state: TState) => TSelector): ObservableState<TSelector>;
    /**
     ...
     */
    memoizeForState<T extends AnyFunction>(func: T, resolver: FunctionWithSameArgs<T>, shouldClear?: () => boolean): ((...args: Parameters<T>) => ReturnType<T>) & Partial<_.MemoizedFunction> & Partial<MemoizeMissHit>;
    /**
     ...
     * @return {*}  {(((...args: Parameters<T>) => ReturnType<T>) & Partial<_.MemoizedFunction> & Partial<MemoizeMissHit>)}
     */
    memoize<T extends AnyFunction>(func: T, resolver: FunctionWithSameArgs<T>): ((...args: Parameters<T>) => ReturnType<T>) & Partial<_.MemoizedFunction> & Partial<MemoizeMissHit>;
    /**
...
export declare type LogSeverity = 'debug' | 'info' | 'event' | 'warning' | 'error' | 'critical';
export declare type LogSpanFlag = 'begin' | 'end';
